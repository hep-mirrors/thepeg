// non-inlined functions of LorentzRSSpinor class
// -*- C++ -*-
//
...
LorentzRSSpinorBar LorentzRSSpinor::bar()
{
...
// boost the spinor
LorentzRSSpinor & LorentzRSSpinor::boost(double bx,double by,double bz)
{
  ...
  *this=LorentzRSSpinor(out[0][0],out[0][1],out[0][2],out[0][3],
			out[1][0],out[1][1],out[1][2],out[1][3],
			out[2][0],out[2][1],out[2][2],out[2][3],
			out[3][0],out[3][1],out[3][2],out[3][3],_type,_dirac);
  return *this;
...
// boost the spinor
LorentzRSSpinor & LorentzRSSpinor::boost(const Hep3Vector & boostvec)
{
...
LorentzRSSpinor & LorentzRSSpinor::transform(const LorentzRotation & r)
{
  unsigned int ix,iy,ixa,iya;
  LorentzRSSpinor out;
  if(Rep()==defaultDRep)
    ...
    {
      LorentzRSSpinor temp=(*this).transformRep(defaultDRep);
      for(ix=0;ix<4;++ix)
	{
	  for(iy=0;iy<4;++iy)
	    {
		...
		{
		  for(iya=0;iya<4;++iya)
		    {out(ix,iy)+=r.one()(ix,ixa)*r.half()(iy,iya)*out(ixa,iya);}
		}
	    }
	}
      out.changeRep(Rep());
